...
- **Restore missing Checkpoint cards and resolve virtual step duplication / 恢复影子 Checkpoint 卡片并解决虚拟步骤冲突**: Newer IDE versions deprecated the USER message injection inside `messagePrompts`, rendering the previous regex parser blank. Additionally, when shadow steps lacked native `stepIdx` values, the key fell back to a shared key (`-1`), causing extractions to override each other. Fix: implemented `extractCheckpointsFromTrajectorySteps(steps)` to parse checkpoints directly from `trajectory.steps` returned by the LSP RPC. Added single-conversation isolation, resolved key duplication by allocating high-range virtual step indices (`100000 + i * 100 + checkpointNumber`), and filtered out the raw virtual step IDs (`>= 100000`) in WebView chips for a cleaner layout.
  新版 IDE 消息列表 `messagePrompts` 废弃了 USER 消息的 Checkpoint 注入导致提取失效。此外，当影子步骤的 `stepIdx` 缺失时，Map 去重 Key 会退化为 `-1` 发生冲突，导致提取的数据在合并时被覆盖。修复：在 `parser.ts` 中实现 `extractCheckpointsFromTrajectorySteps(steps)`，直接从 `trajectory.steps` 中提取意图、总结、请求列表及受影响文件，保留单会话隔离数据；采用虚拟数值区间 `100000 + i * 100 + checkpointNumber` 区分无 ID 步骤解决冲突；在 `activity-panel.ts` 面板渲染时屏蔽对虚拟高区间步骤 `step 100000+` 芯片的展示，优化 UI 呈现。
...
- **Files changed**: 15 (`package.json`, `src/models.ts`, `src/extension.ts`, `src/statusbar.ts`, `src/webview-panel.ts`, `src/webview-settings-tab.ts`, `src/webview-script.ts`, `src/webview-profile-tab.ts`, `src/webview-icons.ts`, `src/activity-panel.ts`, `src/quota-tracker.ts`, `src/daily-archival.ts`, `src/gm/parser.ts`, `src/gm/tracker.ts`, `CHANGELOG-v3.md`)
- **TypeScript compile**: Zero errors
- **Tests**: 70 tests passing (`npm test` / `npx vitest run`)
