...
function buildTimeline(s, currentUsage) {
    ...
    const buildSegmentStats = (actions) => {
        ...
        for (const a of actions) {
            ...
            if (a.category === 'reasoning') {
                reasoningCount++;
                if (a.gmInputTokens) {
                    totalIn += a.gmInputTokens;
                }
                if (a.gmOutputTokens) {
                    totalOut += a.gmOutputTokens;
                }
                if (a.gmThinkingTokens) {
                    totalThinking += a.gmThinkingTokens;
                }
                if (a.gmCacheReadTokens) {
                    totalCache += a.gmCacheReadTokens;
                }
                if (a.gmCredits) {
                    totalCredits += a.gmCredits;
                }
            }
            if (a.model) {
                models.add(a.model);
            ...
            if (a.gmModel) {
                models.add(a.gmModel);
            }
            if (!minTime || (a.timestamp && a.timestamp < minTime)) {
                minTime = a.timestamp;
            }
            if (!maxTime || (a.timestamp && a.timestamp > maxTime)) {
                maxTime = a.timestamp;
            }
        }
        ...
        if (models.size === 1) {
            ...
            model = [...models].filter(m => !m.startsWith('MODEL_PLACEHOLDER')).pop() || [...models][0];
        }
        let durationSec = 0;
        try {
            if (minTime && maxTime) {
    ...
    for (let si = 0; si < reversedSegments.length; si++) {
        ...
        const chips = [];
        if (stats.model) {
            chips.push(`<span class="seg-chip seg-chip-model">${(0, webview_helpers_1.esc)(stats.model)}</span>`);
        }
        if (stats.reasoningCount > 0) {
            chips.push(`<span class="seg-chip seg-chip-calls">${stats.reasoningCount} ${(0, i18n_1.tBi)('calls', '调用')}</span>`);
        }
        ...
        if (stats.totalCache > 0) {
            chips.push(`<span class="seg-chip seg-chip-cache">${fmtTok(stats.totalCache)} ${(0, i18n_1.tBi)('cache', '缓存')}</span>`);
        }
        ...
        const userPreview = segment.user
            ? (0, webview_helpers_1.esc)((segment.user.userInput || segment.user.detail || '').replace(/\s*\n\s*/g, ' '))
            : (0, i18n_1.tBi)('AI actions (no user anchor)', 'AI 动作（缺少用户锚点）');
