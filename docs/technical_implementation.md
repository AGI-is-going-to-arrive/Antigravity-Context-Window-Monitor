...
---
基于 TypeScript 构建，适用于 Antigravity IDE。包含 138 个 vitest 单元测试覆盖纯逻辑函数（`npm test`）：`discovery.test.ts`（50 tests，含 `selectMatchingProcessLine` 多窗口回退 + CJK 路径 + WSL/vscode-remote URI + 退避常量验证）、`statusbar.test.ts`（11 tests）、`tracker.test.ts`（22 tests）、`quota-tracker.test.ts`（27 tests）。
Built with TypeScript for the Antigravity IDE. Includes 138 vitest unit tests covering pure logic functions (`npm test`): `discovery.test.ts` (50 tests, incl. `selectMatchingProcessLine` multi-window fallback + CJK paths + WSL/vscode-remote URIs + backoff constant validation), `statusbar.test.ts` (11 tests), `tracker.test.ts` (22 tests), `quota-tracker.test.ts` (27 tests).
