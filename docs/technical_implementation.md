...
---
基于 TypeScript 构建，适用于 Antigravity IDE。当前共有 153 个 vitest 单元测试（`npm test`），覆盖 discovery 解析、价格表渲染/保存、工具目录清空持久化、模型默认值恢复、quota pool 分组和 contextLimits 迁移等纯逻辑路径。
Built with TypeScript for the Antigravity IDE. The repository currently contains 153 vitest unit tests (`npm test`) covering discovery parsing, pricing table rendering/save behavior, tool catalog clear persistence, model default restoration, quota pool grouping, and contextLimits migration.
