...
│   ├── gm-panel.ts               # GM 数据面板 HTML（费用估算 + 价格参考）
...
面板总框架：标签切换（Monitor / Settings / Activity / Profile / History / GM Data）、CSP 安全策略、消息通信。各标签内容由独立模块生成。

Panel framework: tab switching (Monitor / Settings / Activity / Profile / History / GM Data), CSP policy, message communication. Each tab's content is generated by independent modules.
...
| `gm-panel.ts` | GM Data 标签页 HTML（费用估算 + 价格参考） |
...
生成 GM Data 标签页的完整 HTML。

Generates complete HTML for the GM Data tab.
...
| Pricing | `buildPricingTable` | 动态模型价格参考（自动匹配 / 默认 $0） |

内置 `DEFAULT_PRICING`（per 1M tokens）和 `findPricing()` 三级匹配：精确 → 前缀 → 子串。
